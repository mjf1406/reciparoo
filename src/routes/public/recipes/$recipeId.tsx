...
function PublicRecipeDetailPage() {
    ...
    // Update page metadata (title + description) when recipe loads
    React.useEffect(() => {
        if (!recipe) return;
        ...
        if (recipe.name) document.title = recipe.name;

        const appendedAttr = "data-reciparoo-meta";

        if (descriptionMeta) {
            descriptionMeta.setAttribute("content", recipe.description ?? "");
        } else if (recipe.description) {
            const m = document.createElement("meta");
            m.name = "description";
            m.content = recipe.description;
            m.setAttribute(appendedAttr, "1");
        ...
        return () => {
            ...
            if (descriptionMeta) {
                ...
                else descriptionMeta.removeAttribute("content");
            } else {
                const appended = document.querySelector(
                    `meta[${appendedAttr}="1"]`
                );
